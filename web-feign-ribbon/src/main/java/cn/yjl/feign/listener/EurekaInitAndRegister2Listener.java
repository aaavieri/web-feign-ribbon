 ...
 *
 */
public class EurekaInitAndRegister2Listener implements ServletContextListener {

	/** config.properties文件内容 */
	private static final DynamicPropertyFactory configInstance = DynamicPropertyFactory
			.getInstance();
	
	private static final Logger log = LoggerFactory.getLogger(EurekaInitAndRegister2Listener.class);
	...
	private void initFeignClient() {
		...
		timer.schedule(new TimerTask() {
			public void run() {
				EurekaInitAndRegister2Listener.this.setServiceInfo(serviceList);
			}
		}, 0L, configInstance.getLongProperty("eureka.refresh.peroid", 300000L).get());
	}
